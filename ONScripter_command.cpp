...
int ONScripter::nextcselCommand()
{
    ...
    if (last_nest_info != &root_nest_info && last_nest_info->nest_mode == NestInfo::LABEL){
...
#if defined(MACOSX) /* convert sjis to utf-8 */
...
{
    int no = script_h.readInt();
    if (no < 0 || no >= MAX_SPRITE_NUM || sprite_info[no].image_surface == NULL){
        for (int i=0 ; i<3 ; i++)
            if ( script_h.getEndStatus() & ScriptHandler::END_COMMA ) script_h.readStr();
        return RET_CONTINUE;
    }
...
int ONScripter::bltCommand()
{
    ...
    if ( sw == dw && sw > 0 && sh == dh && sh > 0 ){
        ...
        dst_rect.x = dst_rect.x * device_width / screen_width;
        dst_rect.y = dst_rect.y * device_width / screen_width;
        dst_rect.w = dst_rect.w * device_width / screen_width;
        dst_rect.h = dst_rect.h * device_width / screen_width;
#if defined(IOS)
        if(resize){
            if(space_up){
                dst_rect.x = dst_rect.x * device_width / screen_width;
                dst_rect.y = dst_rect.y * device_width / screen_width - space_size;
                dst_rect.w = dst_rect.w * device_width / screen_width;
                dst_rect.h = dst_rect.h * device_width / screen_width - space_size * 2;
            }else{
                dst_rect.x = dst_rect.x * device_width / screen_width - space_size;
                dst_rect.y = dst_rect.y * device_width / screen_width;
                dst_rect.w = dst_rect.w * device_width / screen_width - space_size * 2;
                dst_rect.h = dst_rect.h * device_width / screen_width;
            }
        }
#endif
