 ...
 *
 *  Copyright (c) 2016 Marron Akanishi. All rights reserved.
 *
 *  marron.general@gmail.com
 *
...
#define SPACE_LEFT_FILE "space_left.png"
...
void ONScripter::initSDL(){
    ...
    if(((float)device_width / device_height) != ((float)screen_width / screen_height)){
        ...
        resize = true;
        space_up = true;
        float check = (float)device_width / screen_width;
        if(device_height <= screen_height * check){
            space_up = false;
            printf("space side\n");
        }
        if(space_up){
            float height = screen_height * check;
            space_size = (device_height - height) / 2;
            screen_device_height -= space_size * 2;
            printf("image size:%i\n",space_size * screen_width / device_width);
        }else{
            check = (float)device_height / screen_height;
            float width = screen_width * check;
            space_size = (device_width - width) / 2;
            screen_device_width -= space_size * 2;
...
ONScripter::ONScripter()
{
	...
	// External Players
	midi_cmd = getenv("MUSIC_CMD"); // Where is this function?
...
int ONScripter::init()
{
	...
	if(resize){
		if(space_up) {
			//0->top 1->bottom
			back_surface[0]  = AnimationInfo::allocSurface( screen_width, (float)space_size * screen_width / device_width, texture_format );
			back_surface[1]  = AnimationInfo::allocSurface( screen_width, (float)space_size * screen_width / device_width, texture_format );
		}else{
			//0->left 1->right
			back_surface[0]  = AnimationInfo::allocSurface( (float)space_size * screen_height / device_height, screen_height, texture_format );
			back_surface[1]  = AnimationInfo::allocSurface( (float)space_size * screen_height / device_height, screen_height, texture_format );
		}
	}
#endif
#if defined(TVOS)
...
void ONScripter::flushDirect( SDL_Rect &rect, int refresh_mode )
{
	...
	if(resize){
		if(space_up){
			dst_rect.x = 0;
			dst_rect.y = space_size;
			dst_rect.w = device_width;
			dst_rect.h = device_height - space_size * 2;
		}else{
			dst_rect.x = space_size;
			dst_rect.y = 0;
			dst_rect.w = device_width - space_size * 2;
...
#if defined(IOS)
	if(resize){
		SDL_Rect space_rect[2];
		space_rect[0].x = 0;
		space_rect[0].y = 0;
		if(space_up){
			space_rect[0].w = device_width;
			space_rect[0].h = space_size;
			space_rect[1].x = 0;
			space_rect[1].y = device_height - space_size;
			space_rect[1].w = device_width;
			space_rect[1].h = space_size;
		}else{
			space_rect[0].w = space_size;
			space_rect[0].h = device_height;
			space_rect[1].x = device_width - space_size;
			space_rect[1].y = 0;
			space_rect[1].w = space_size;
			space_rect[1].h = device_height;
		}
